...
type EngineCallback<T> = Box<dyn Fn(&Arc<T>)>;
...
pub struct EngineCallbacks<T> {
    on_engine_running: Option<EngineCallback<T>>,
    on_exit: Option<EngineCallback<T>>,
}
...
impl<T: Default> EngineBuilder<T> {
    ...
    pub fn add_on_engine_running<U: 'static + Fn(&Arc<T>)>(
        mut self,
        on_engine_running: U,
    ) -> Self {
...
impl<T> EngineCtx<T> {
    ...
    unsafe extern "C" fn dispatch_on_engine_running(context: *mut c_void) {
        let context = (context as *mut EngineCtx<T>).as_ref().unwrap();
        if let Some(on_engine_running) = &context.on_engine_running {
            on_engine_running(&context.inner);
        }
    }
    ...
    unsafe extern "C" fn dispatch_on_exit(context: *mut c_void) {
        let context = (context as *mut EngineCtx<T>).as_ref().unwrap();
        if let Some(on_exit) = &context.on_exit {
            on_exit(&context.inner);
        }
    }

    fn as_envoy_engine_callbacks(&self) -> envoy_mobile_sys::envoy_engine_callbacks {
        envoy_mobile_sys::envoy_engine_callbacks {
            on_engine_running: Some(Self::dispatch_on_engine_running),
            on_exit: Some(Self::dispatch_on_exit),
            context: Box::into_raw(Box::new(self.inner.clone())) as *mut c_void,
        }
    }
}
